...
getImageSize(){
    ...
    Orca)
        cp_arr=(l_pmsys l_agdsp)
        cpname_arr=($DFS $DSP_LTE_AG);;
    *)
    ...
    do
        ...
        if [ $value = $1 ]; then
            ...
            size_ab=`awk '/Partition id="'${cp_arr[i]}_a'"/{print $3}' $PAC_CONFILE | cut -d\" -f2`
            echo "size = $size"
            echo "size_ab = $size_ab"
...
doAvbSignImage(){
    ...
    for loop in $@;do
        ...
        if [ -z "$size_ab" ];then
            echo "size_ab is null"
        else
            if [ $size_ab -gt 0 ];then
                sizeInByte=$(expr $size_ab \* $MB)
            else
                echo "size_ab($size_ab) is wrong"
                sizeInByte=0
                continue
            fi
        fi
...
doSprdModemSignImage(){    
    ...
    for loop in $@;do
        if [ -f $loop ] ; then
            ...
            if [[ "$CPBASE" == "Orca" ]];then
                $STOOL_PATH/imgheaderinsert $loop 0 0
                system_return $? imgheaderinsert
                $STOOL_PATH/sprd_sign $(getRawSignImageName $loop) $SPRD_CONFIG_FILE pss
            else
...
doInit_v1(){
    ...
    Orca)
        echo "orca series"
        CPBASE=Orca
        DSP_W_G=${IMG_DIR}/${PROJ}/$(getImageName $MODEM_DSP_W_G)
        MODEM_W=${IMG_DIR}/${PROJ}/$(getImageName $MODEM_MODEM)
        DSP_W_L=${IMG_DIR}/${PROJ}/$(getImageName $MODEM_DSP_W_L)
        DSP_LTE_LDSP=${IMG_DIR}/${PROJ}/$(getImageName $MODEM_G_DSP)


        cp $MODEM_DSP_W_G   ${IMG_DIR}/${PROJ}/ -rfv
        system_return $? $MODEM_DSP_W_G
        cp $MODEM_MODEM   ${IMG_DIR}/${PROJ}/ -rfv
        system_return $? $MODEM_MODEM
        cp $MODEM_DSP_W_L   ${IMG_DIR}/${PROJ}/ -rfv
        system_return $? $MODEM_DSP_W_L
        cp $MODEM_G_DSP   ${IMG_DIR}/${PROJ}/ -rfv
        system_return $? $MODEM_G_DSP
        
        images="$DSP_W_G $MODEM_W $DSP_W_L $DSP_LTE_LDSP"
    ;;
    *)
...
doInit_v2(){
    ...
    case $ChipType in
    sharkl2|sharkl3|sharkle|sl8541e|sharkl6)
        echo "shark series"
        CPBASE=shark
        DFS=${IMG_DIR}/${PROJ}/$(getImageName $MODEM_DFS)
        MODEM_LTE=${IMG_DIR}/${PROJ}/$(getImageName $MODEM_MODEM)
    ...
    Orca)
        ...
        doModemSignImage $DFS $DSP_LTE_AG
    ;;
    *)
        echo_exit "[== not support for $ChipType ==]"
        exit;;
...
while getopts ":c:m:d:g:l:a:x:t:w:" opt;do
    case $opt in
        c)
        ...
        ;;
        t)
            export MODEM_DSP_W_G=$OPTARG
            echo_yellow "\n-t MODEM_DSP_W_G"
            echo_eval MODEM_DSP_W_G
        ;;
        w)
            export MODEM_DSP_W_L=$OPTARG
            echo_yellow "\n-w MODEM_DSP_W_L"
            echo_eval MODEM_DSP_W_L
        ;;
        \?)
            echo -e "\n[parameters error!]\n"
            exit 1

...
############################################################################################################
    ...
    if [ $PLATFORM_SDK_VERSION  -lt 29 ];then
	    gms_flag
    fi
	
	CPU_COUNTS=$(cat /proc/cpuinfo | grep -i 'processor' | wc -l)
	CPU_COUNT=$((CPU_COUNTS/2))
	...
	if [ $PLATFORM_SDK_VERSION -gt 27 ];then
		...
		kheader
		make api-stubs-docs-update-current-api -j$CPU_COUNT
		make $carrier_param -j$CPU_COUNT
		if [ $PLATFORM_SDK_VERSION  -gt 28 ];then
            transunsparse
		fi
	else
		source build/envsetup.sh
...
############################################################################################################
			...
			grep -rin "ro.internal.display.id"  $OUT_DIR/system/build.prop 2>&1 > /dev/null
			if [ $? -eq 0 ]; then
			    echo "****ro.internal.display.id  not null ***"
			     _type1=`cat $OUT_DIR/system/build.prop | grep "ro.internal.display.id="`
			     _type1_temp=${_type1##ro.internal.display.id=}
			else
			 echo "****ro.internal.display.id null***"
			     _type1_temp=${Project}_${modem_config}
			fi

	                build_type1=`cat $OUT_DIR/system/build.prop | grep "ro.build.type="`
	                build_type1_temp=${build_type1##ro.build.type=}

	                PAC_VERSION="S813_JingPad_C1"_${_type1_temp}_${build_type1_temp}_$(date "+%Y%m%d%H%M")
			chmod a+x $Script_path/*
			perl $Script_path/mkpac.pl "${PAC_VERSION}.pac" "flash.cfg" "FlashParam"
			
			if [ -e $Script_path/../ImageFiles/${PAC_VERSION}.pac ];then
				cp $Script_path/../ImageFiles/${PAC_VERSION}.pac $Script_path/../$Project/$modem_config/
				echo "${PAC_VERSION}.pac [PASS]">>$Script_path/../build_pac.log
				echo -e "\n\n${PAC_VERSION}.pac [PASS]\n\n"
			else
				echo "${Project}_${modem_config}.pac [FAIL]">>$Script_path/../build_pac.log
...
function Write_Sign(){
    ...
    while read line ;do
        ...
        if [[ "$modem_key" == "DFS" ]] || [[ "$modem_key" == "Modem_L_PMSYS" ]];then
    ...
    do
        ...
        if [[ $param_key = "DSP_GSM" ]] || [[ $param_key = "DSP_LTE_GGE" ]] || [[ $param_key = "DSP_WLTE_GGE" ]] || [[ $param_key = "DSP_TDLTE_TD" ]] || [[ $param_key = "Modem_NR_DSP1" ]]; then
...
#    fi
    ...
    do
        ...
        case $param_key in
            'Modem_W'|'Modem_LTE'|'Modem_WLTE'|'Modem_TDLTE'|'Modem_NR')
...
function SIGN(){
    ...
    PLATFORM_VERSION=$(grep "PLATFORM_VERSION.[a-Z][a-Z][a-Z][0-9] :="  $code_path/build/core/version_defaults.mk |awk -F "= " '{print $NF}')
    ...
    while read line ;do
        ...
        case $image_key in
            ...
            ;;
            'DSP_GSM'|'DSP_LTE_GGE'|'DSP_WLTE_GGE'|'DSP_TDLTE_TD'|'Modem_NR_DSP1')
                #-g
                MODEM_G_DSP=" -g $image"
                
                MODEM_G_DSP_KEY=$image
            ...
            'Modem_NR_V3PHY')
                #-t
                MODEM_DSP_W_G=" -t $image"
                
                MODEM_DSP_W_G_KEY=$image

                MODEM_DSP_W_G_IMAGE=$(getImageName $image)
                echo "$image_key=$MODEM_DSP_W_G_IMAGE">>$Script_path/../$Project/$modem_config/.cfg_check
            ;;
            'Modem_NR_PHY')
                #-w
                MODEM_DSP_W_L=" -w $image"
                
                MODEM_DSP_W_L_KEY=$image

                MODEM_DSP_W_L_IMAGE=$(getImageName $image)
                echo "$image_key=$MODEM_DSP_W_L_IMAGE">>$Script_path/../$Project/$modem_config/.cfg_check
            ;;
            'Modem_NR_AGDSP')
                #9.0 -n /8.1 -a
                if [[ "$PLATFORM_VERSION" == "8.1.0" ]];then
                    MODEM_DSP_AG=" -a $image"
                    ...
                    MODEM_DSP_N_A=" -n $image"
                
                    MODEM_DSP_N_A_KEY=$image

                    MODEM_DSP_N_A_IMAGE=$(getImageName $image)
                    echo "$image_key=$MODEM_DSP_N_A_IMAGE">>$Script_path/../$Project/$modem_config/.cfg_check
                fi
            ;;
            'Modem_NR_PMSYS')
                #9.0 -u /8.1 -d
                if [[ "$PLATFORM_VERSION" == "8.1.0" ]];then
                    ...
                    MODEM_N_DFS_KEY=$image

                    MODEM_N_DFS_IMAGE=$(getImageName $image)
                    echo "$image_key=$MODEM_N_DFS_IMAGE">>$Script_path/../$Project/$modem_config/.cfg_check
                fi
            ;;
            *)
                #nv
    ...
    echo_eval MODEM_MODEM_KEY
    ChipType=`strings $MODEM_MODEM_KEY | grep "Project Version:" |awk '{print $NF}'|awk -F "_"  '{print$1}'`
    echo_eval ChipType
    #########用作验证签名########################################
    if [[ "$ChipType" == "Orca" ]];then
        PLATFORM_VERSION=$(grep "PLATFORM_VERSION.[a-Z][a-Z][a-Z][0-9] :="  $code_path/build/core/version_defaults.mk |awk -F "= " '{print $NF}')
        echo "PLATFORM_VERSION:$PLATFORM_VERSION"
        if [[ "$PLATFORM_VERSION" == "8.1.0" ]];then
            sign_param=($MODEM_MODEM_IMAGE $MODEM_DSP_W_G_IMAGE $MODEM_DSP_W_L_IMAGE $MODEM_G_DSP_IMAGE)
            unsign_param=($MODEM_DSP_KEY $MODEM_DSP_LTE_CDMA_KEY $MODEM_DFS_KEY $MODEM_DSP_AG_KEY $MODEM_DSP_N_A_KEY $MODEM_N_DFS_KEY)
        else
            sign_param=($MODEM_DFS_IMAGE $MODEM_DSP_AG_IMAGE)
            unsign_param=($MODEM_MODEM_KEY $MODEM_DSP_KEY $MODEM_G_DSP_KEY $MODEM_DSP_LTE_CDMA_KEY $MODEM_DSP_W_L_KEY $MODEM_DSP_W_G_KEY $MODEM_DSP_N_A_KEY $MODEM_N_DFS_KEY)
        fi
    elif [[ "$ChipType" == "pike2" ]];then
        sign_param=($MODEM_DFS_IMAGE $MODEM_MODEM_IMAGE $MODEM_G_DSP_IMAGE)
        unsign_param=($MODEM_DSP_KEY $MODEM_DSP_LTE_CDMA_KEY $MODEM_DSP_AG_KEY)
    else
    ...
    ./avb_sign_modem_v3.sh $PRODUCT_XML$MODEM_MODEM$MODEM_DSP$MODEM_G_DSP$MODEM_DSP_AG$MODEM_DFS$MODEM_DSP_LTE_CDMA$MODEM_DSP_W_G$MODEM_DSP_W_L
    ##############################################################################
    ...
    if [ -f "$SPRD_SECURE_FILE" ]; then
        ...
        #拷贝已签名bin文件
        copy_sign_image
        echo "unsign_param:$unsign_param"
        unsign_length=${#unsign_param[@]}
        echo -e "[==unsign_param:${#unsign_param[@]}==]"
        if [[ "$unsign_length" == "0" ]];then
            echo -e "[==not find unsign_param,do nothing==]"
        else
            echo -e "[==copy unsign_param modem==]"
            for (( i=0; i<$unsign_length; i++ ));do
                image=${unsign_param[i]}
                echo_eval image
                cp -rf $image $Script_path/../ImageFiles
            done
        fi
...
function Modem_Copy_Orca_8(){
    ...
    while read line ;do
        ...
        case $image_key in
            'Modem_NR')
                #-m
                cp $image $MODEM_BIN_PATH/nrmodem.bin
            ;;
            'Modem_NR_DSP1')
                #-g
                cp $image $MODEM_BIN_PATH/nrdsp1.bin
            ;;
            'Modem_NR_PMSYS')
                #-d
                cp $image $MODEM_BIN_PATH/pmsys.bin
            ...
            'Modem_NR_V3PHY')
                #-t
                cp $image $MODEM_BIN_PATH/nrv3phy.bin
            ;;
            'Modem_NR_PHY')
                #-w
                cp $image $MODEM_BIN_PATH/nrphy.bin
            ;;
            *)
                #nv
                if [[ "$image" =~ "nvitem" ]];then
                    cp $image $MODEM_BIN_PATH/nrnvitem.bin
                fi
        ;;
        esac
    done < $Script_path/../$Project/$modem_config/.sign
...
function Modem_Copy_Orca(){
    ...
    while read line ;do
        ...
        case $image_key in
            'Modem_NR')
                #-m
                cp $Script_path/../$Project/$modem_config/$image $MODEM_BIN_PATH/nrmodem.bin
            ;;
            'Modem_NR_DSP1')
                #-g
                cp $Script_path/../$Project/$modem_config/$image $MODEM_BIN_PATH/nrdsp1.bin
            ;;
            'Modem_NR_AGDSP')
                #-n
                cp $Script_path/../$Project/$modem_config/$image $MODEM_BIN_PATH/nragdsp.bin
            ;;
            'MODEM_L_AGDSP')
                #-a
                cp $Script_path/../$Project/$modem_config/$image $MODEM_BIN_PATH/lteagdsp.bin
            ;;
            'Modem_NR_PMSYS')
                #-nd
                cp $Script_path/../$Project/$modem_config/$image $MODEM_BIN_PATH/nrpmsys.bin
            ;;
            'Modem_NR_V3PHY')
                #-t
                cp $Script_path/../$Project/$modem_config/$image $MODEM_BIN_PATH/nrv3phy.bin
            ;;
            'Modem_NR_PHY')
                #-w
                cp $Script_path/../$Project/$modem_config/$image $MODEM_BIN_PATH/nrphy.bin
            ;;
            'Modem_L_PMSYS')
                #-d
                cp $Script_path/../$Project/$modem_config/$image $MODEM_BIN_PATH/ltepmsys.bin
            ;;
            *)
                #nv
                if [[ "$image" =~ "nvitem" ]];then
                    cp $Script_path/../$Project/$modem_config/$image $MODEM_BIN_PATH/nrnvitem.bin
                fi

                if [[ "$image" =~ "deltanv" ]];then
                    cp $Script_path/../$Project/$modem_config/$image $MODEM_BIN_PATH/nrdeltanv.bin
                fi
        ;;
        esac
    done < $Script_path/../$Project/$modem_config/.sign
    ...
    while read line ;do
        ...
        case $modem_key in
            ...
            ;;
            'Modem_NR_UBOOT')
                cp $Script_path/../$Project/$modem_config/$modem_name $MODEM_BIN_PATH/nruboot.bin
            ;;
            'Modem_NR_SPL')
                cp $Script_path/../$Project/$modem_config/$modem_name $MODEM_BIN_PATH/nrspl.bin
            ;;
            'Modem_NR_SML')
                cp $Script_path/../$Project/$modem_config/$modem_name $MODEM_BIN_PATH/nrsml.bin
            ;;
            'Modem_NR_BOOT')
                cp $Script_path/../$Project/$modem_config/$modem_name $MODEM_BIN_PATH/nrboot.bin
            ;;
            *)
        ;;
        esac
...
############################################################################################################
    ...
    echo -e "[PLATFORM_SDK_VERSION:$PLATFORM_SDK_VERSION]"
    ...
    for flash_cfg in `cd $Script_path/../$Project && find -maxdepth 2 -name flash.cfg`;do
        ...
        if [[ "$ChipType" == "Orca" ]];then
            ...
            echo "PLATFORM_VERSION:$PLATFORM_VERSION"
            if [[ "$PLATFORM_VERSION" == "8.1.0" ]];then
                echo -e "[Modem_Copy_Orca_8]"
                Modem_Copy_Orca_8
            else
                echo -e "[Modem_Copy_Orca]"
                Modem_Copy_Orca
            fi
        else
            echo -e "[Modem_Copy]"
